...
class DashboardMetrics(BaseModel):
    ...
    completed_reviews: int = Field(
        ..., description="Number of successfully completed reviews"
    )
    open_tickets: int = Field(
        ..., description="Number of currently open/unresolved tickets"
    )
    critical_findings: int = Field(
        ..., description="Number of critical issues detected"
    )
    average_quality: float = Field(
        ..., description="Average code quality score (0-100)"
    )
    language_distribution: dict[str, int] = Field(
        ..., description="Count of reviews grouped by programming language"
    )
...
class ReviewSummary(BaseModel):
    ...
    quality_score: int = Field(
        ..., description="Dynamic quality score calculated from findings"
    )
    findings_count: int = Field(..., description="Total findings count")
    open_tickets_count: int = Field(
        ..., description="Count of open tickets associated with this review"
    )
...
class TicketSummary(BaseModel):
    ...
    priority: str = Field(
        ..., description="Priority level of the ticket (e.g., P0, P1, P2, P3)"
    )
    status: str = Field(
        ..., description="Resolution status (e.g., OPEN, IN_PROGRESS, etc.)"
    )
