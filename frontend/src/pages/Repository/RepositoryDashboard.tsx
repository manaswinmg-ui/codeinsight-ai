...
interface RepositoryDetail {
    ...
    most_problematic_files: Array<{ file_path: string; findings_count: number; quality_score: number }>;
}
...
export const RepositoryDashboard: React.FC<RepositoryDashboardProps> = ({ onSelectReview }) => {
    ...
    const fetchRepoList = async () => {
        setLoadingList(true);
        try {
            const response = await fetch(`${getBaseUrl()}/api/v1/repositories?page=1&limit=50`);
    ...
    useEffect(() => {
        ...
        const interval = setInterval(async () => {
            try {
                const response = await fetch(`${getBaseUrl()}/api/v1/repositories/${activeRepoId}`);
                if (response.ok) {
    ...
    if (activeRepoId !== null) {
        ...
        const isProcessing = repoDetail.status === 'PENDING' || repoDetail.status === 'PROCESSING';
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '16px' }}>
                    <div>
                        ...
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.85rem', marginTop: '6px', margin: 0 }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                        <span
                            style={{
                                ...
                                fontSize: '0.8rem',
                                fontWeight: 700,
                                background: repoDetail.status === 'COMPLETED' ? 'rgba(16, 185, 129, 0.15)' : 'rgba(239, 68, 68, 0.15)',
                                color: repoDetail.status === 'COMPLETED' ? 'var(--success-color)' : 'var(--danger-color)',
                            }}
                        >
                            {repoDetail.status}
                        </span>
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '32px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                    ...
                    <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginTop: '6px', margin: 0 }}>
                        Review, aggregate, and audit quality insights for entire software projects.
                    </p>
                </div>
            ...
            {error && (
                <div style={{ padding: '12px 16px', background: 'rgba(239, 68, 68, 0.1)', color: 'var(--danger-color)', borderRadius: '8px', fontSize: '0.9rem', fontWeight: 600 }}>
                    ⚠️ {error}
                </div>
            )}
            ...
            >
                {loadingList ? (
                    ...
                    <div style={{ padding: '60px 40px', textAlign: 'center' }}>
                        ...
                        <p style={{ fontWeight: 600, fontSize: '1.1rem', marginBottom: '8px' }}>No scanned repositories yet</p>
                        <p style={{ color: 'var(--text-secondary)', fontSize: '0.9rem', marginBottom: '20px' }}>
                            Upload your codebase in a ZIP format to evaluate files and detect critical engineering issues.
                        </p>
                        <button
                            onClick={() => setShowUpload(true)}
                            style={{
                    ...
                    <div style={{ overflowX: 'auto' }}>
                        <table style={{ width: '100%', borderCollapse: 'collapse', textAlign: 'left' }}>
                            ...
                            <tbody>
                                {repoList.map((repo) => {
                                    const qualityColor = repo.overall_quality !== null
                                        ? repo.overall_quality >= 80 ? 'var(--success-color)' : repo.overall_quality >= 50 ? '#f59e0b' : 'var(--danger-color)'
                                        : 'var(--text-secondary)';
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <td style={{ padding: '16px', fontWeight: 700, color: qualityColor }}>
                                                {repo.overall_quality !== null ? `${repo.overall_quality}/100` : '-'}
                                            </td>
                                            <td style={{ padding: '16px' }}>
                                                <span
                                                    style={{
                                                        ...
                                                        fontSize: '0.75rem',
                                                        fontWeight: 600,
                                                        background: repo.status === 'COMPLETED' ? 'rgba(16, 185, 129, 0.1)' : repo.status === 'PROCESSING' ? 'rgba(99, 102, 241, 0.1)' : 'rgba(239, 68, 68, 0.1)',
                                                        color: repo.status === 'COMPLETED' ? 'var(--success-color)' : repo.status === 'PROCESSING' ? 'var(--accent-color)' : 'var(--danger-color)',
                                                    }}
                                                >
                                                    {repo.status}
                                                </span>
