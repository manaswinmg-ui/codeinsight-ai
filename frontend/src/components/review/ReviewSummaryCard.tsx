...
export const ReviewSummaryCard: React.FC<ReviewSummaryCardProps> = ({ reviewData }) => {
  ...
  const getRecommendation = (score: number) => {
    if (score >= 90) return { text: 'APPROVED — Strong Code Quality', color: 'var(--success-color)' };
    if (score >= 70) return { text: 'APPROVED — Minor improvements suggested', color: 'var(--warning-color)' };
    if (score >= 50) return { text: 'REFACTOR RECOMMENDED — Critical issues found', color: '#fbbf24' };
    return { text: 'NOT APPROVED — Needs major rewrite', color: 'var(--danger-color)' };
  };
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 2fr', gap: '20px' }}>
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
            ...
            <div style={{ fontSize: '1.2rem', fontWeight: 800, color: recommendation.color }}>
              {recommendation.text}
            </div>
          </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '16px' }}>
        <StatItem label="Language" value={reviewData.language.toUpperCase()} icon="🌐" />
        ...
        <StatItem label="Critical Findings" value={criticalFindings} icon="⚠️" />
        <StatItem label="Open Tickets" value={openTickets} icon="🎫" />
        <StatItem label="Static Analysis (Ruff)" value={staticCount} icon="⚡" />
